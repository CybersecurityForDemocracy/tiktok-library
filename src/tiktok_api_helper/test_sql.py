...
def test_upsert_no_prior_insert(test_database_engine, mock_videos, mock_crawl):
    ...
    with Session(test_database_engine) as session:
        ...
        assert [
            {*v.hashtag_names} for v in session.scalars(select(Video).order_by(Video.id)).all()
        ] == [
            {"hashtag1", "hashtag2"},
            {"hashtag1", "hashtag2"},
        ]
...
def test_upsert_updates_existing_and_inserts_new_video_data_and_effect_id(
    ...
    with Session(test_database_engine) as session:
        ...
        assert {
            v.id: v.effect_ids for v in session.scalars(select(Video).order_by(Video.id)).all()
        } == {
