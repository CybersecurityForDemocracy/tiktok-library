...
import datetime
import logging
from typing import Any, Optional,Mapping, Set, Sequence
import json
...
def upsert_videos(
    ...
    with Session(engine) as session:
        ...
        logging.warning("video_id_to_video: %s", video_id_to_video)
        ...
        for each in session.scalars(
            ...
            logging.warning("before merge: %s", new_vid)
            session.merge(new_vid)
            logging.warning("after merge: %s", new_vid)
