...
def generate_query(region_codes: Optional[List[SupportedRegions]] = None,
                   ...
                   exclude_all_hashtags: Optional[str] = None) -> Query:
    query_args = {}
    include_hashtags_condition = None
    if include_any_hashtags:
        query_args["and_"] = [Cond(
            Fields.hashtag_name, sorted(get_normalized_hashtag_set(include_any_hashtags)), Op.IN)]
    elif include_all_hashtags:
        query_args["and_"] = [
            Cond(Fields.hashtag_name, hashtag_name, Op.EQ)
            for hashtag_name in sorted(get_normalized_hashtag_set(include_all_hashtags))]

    if exclude_any_hashtags:
        query_args["not_"] = [Cond(
            Fields.hashtag_name, sorted(get_normalized_hashtag_set(exclude_any_hashtags)), Op.IN)]
    elif exclude_all_hashtags:
        query_args["not_"] = [
            Cond(Fields.hashtag_name, hashtag_name, Op.EQ)
            for hashtag_name in sorted(get_normalized_hashtag_set(exclude_all_hashtags))]

    if region_codes:
        region_code_condition = Cond(Fields.region_code, sorted(region_codes), Op.IN)
        if "and_" in query_args:
            query_args["and_"].append(region_code_condition)
        else:
            query_args["and_"] = region_code_condition
